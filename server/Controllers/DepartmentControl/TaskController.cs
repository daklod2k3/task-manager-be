...
namespace server.Controllers.DepartmentControl;
...
public class TaskController : Controller
{
    ...
    [HttpGet]
    public ActionResult<IEnumerable<TaskEntity>> Get( [FromQuery] long department_id, [FromQuery(Name = "filter")] string? filterString, int? page,
        int? pageSize, string? includes = "", string? orderBy = null)
    {
        ...
        return new SuccessResponse<IEnumerable<TaskEntity>>(
            _repository.Get(CompositeFilter<TaskEntity>.ApplyFilter(filter), includes, orderBy, page, pageSize, _repository.GetByDepartmentId(department_id)));
    }
